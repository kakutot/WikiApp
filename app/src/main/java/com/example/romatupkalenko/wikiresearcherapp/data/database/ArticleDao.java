...
@Query("SELECT * FROM " + ArticleEntry.tableName)
...
@Query("SELECT * FROM " + ArticleEntry.tableName +  " WHERE is_faved = 0" +  " LIMIT :limit OFFSET :offset")
List<ArticleEntry> getNotFavArticlesLimitOffset(int limit,int offset);
...
@Query("UPDATE " + ArticleEntry.tableName + " SET is_faved = 0 WHERE page_id = :pageId")
void unfavArticle(int pageId);
}