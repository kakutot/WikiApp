...
public class PageKeyedArticleDataSource extends PageKeyedDataSource<Integer, ArticleEntry> {
    ...
    public PageKeyedArticleDataSource(Context context,String searchTitle){
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, ArticleEntry> callback) {
        ...
        List<ArticleEntry> articleResponses;
            ...
            public void onResponse(Call<MySearchResponse> call, Response<MySearchResponse> response) {
                ...
                //[Check if article is already saved to favourite
                        articleDao.getAllFavArticles().observeForever(newArticles->{
                            if(newArticles!=null && newArticles.size()>0){
                                for(ArticleEntry articleEntry : newArticles){
                                    if(articleEntry.getPageId()==articleResponse.getPageId()){
                                        articleResponse.setFaved(articleEntry.isFaved());
                                        break;
                                    }
                                }
                            }
                        ...
                        /*Requires API 24+
                        Stream<ArticleEntry> stream = collection.stream();
                        ArticleEntry articleEntry =
                                stream.filter( (article)-> article.getPageId()==articleResponse.getPageId()).findFirst().get();
                       */
